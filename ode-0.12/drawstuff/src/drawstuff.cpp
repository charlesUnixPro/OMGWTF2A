...
static void drawFace (float r, float zoffset)
{
  int i;
  ...
  ny=1; nz=0;		  // normal vector = (0,ny,nz)
  glBegin (GL_TRIANGLE_FAN);
  glNormal3d (0,0, zoffset < 0.0 ? -1 : 1);
  glTexCoord2f (0.5, 0.5);
  glVertex3d (0,0,zoffset);
  for (i=0; i<=n; i++) {
    if (i==1 || i==n/2+1)
      setColor (color[0]*0.75f,color[1]*0.75f,color[2]*0.75f,color[3]);
    glTexCoord2f (0.5 * ny + 0.5, 0.5 * nz + 0.5);
    glNormal3d (0,0, zoffset < 0.0 ? -1 : 1);
...
static Texture *heads_texture = 0;
static Texture *tails_texture = 0;

static Texture *texture[6+1]; // +1 since index 0 is not used
...
void dsStartGraphics (int width, int height, dsFunctions *fn)
{
  ...
  strcpy (s,prefix);
  strcat (s,"/heads.ppm");
  texture[DS_CHECKERED] = heads_texture = new Texture (s);

  strcpy (s,prefix);
  strcat (s,"/tails.ppm");
  texture[DS_CHECKERED] = tails_texture = new Texture (s);
}
...
void dsStopGraphics()
{
  ...
  delete wood_texture;
  delete heads_texture;
  delete tails_texture;
  sky_texture = 0;
  ground_texture = 0;
  wood_texture = 0;
}
				...
				float length, float radius)
{
  ...
  if (dsGetTextures ()) {
    glEnable (GL_TEXTURE_2D);
    dsSetTexture (DS_HEADS);
    heads_texture->bind (0);
  }
drawFace (radius, length / 2.0);
  if (dsGetTextures ()) {
    glEnable (GL_TEXTURE_2D);
    dsSetTexture (DS_TAILS);
    tails_texture->bind (0);
  }
drawFace (radius, - length / 2.0);
  glPopMatrix();
